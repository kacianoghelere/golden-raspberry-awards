...
const WinIntervalInfo: React.FC<Props> = ({
  ...
  const { followingWin, interval, previousWin, producer } = intervalInfo

  const intervalLabel = interval === 1 ? '1 year' : `${interval} years`
    ...
    >
      ...
      >
        ...
        <GS.HStack
          ...
          space='md'
        >
          <GS.Text testID="producer-name">{producer}</GS.Text>
          <GS.Text testID="win-interval">
            {`${intervalLabel} (${previousWin}~${followingWin})`}
          </GS.Text>
        </GS.HStack>
      </GS.Box>
    </GS.VStack>
